...
Figure 3 presents the mean absolute error (MAE) of both forecasting approaches as a function of the training window size, i.e., the number of historical observations available at the time of prediction. The horizontal reference line at MAE = 0 represents a perfect prediction scenario, where the estimated remaining time exactly matches the observed threshold-crossing time.
